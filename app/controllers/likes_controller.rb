class LikesController < ApplicationController
  ...
  def create
    ...
    the_like.song_id = params.fetch("query_song_id")

    if the_like.valid?
      the_like.save
      redirect_to("/user/" + @current_user.id.to_s, { :notice => "Like created successfully." })
    else
      redirect_to("/user/" + @current_user.id.to_s, { :notice => "Like failed to create successfully." })
    end
  end
  ...
  def destroy
    ...
    redirect_to("/user/" + @current_user.id.to_s, { :notice => "Like deleted successfully."} )
  end
end
